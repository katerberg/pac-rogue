...
| `deathsHarvest`     | Death’s Harvest | Getting caught harvests every pellet/power pellet within `DEATHS_HARVEST_RADIUS_TILES` (10) tiles of the player first; if that empties the board it counts as a level clear instead of a death (see [Death’s Harvest](#deaths-harvest) below)                                                                                                                                                                                                                                                                                                                                           |
...
| `tunnelDash`        | Tunnel Dash     | Entering either tunnel's slow-band (the same band ghosts slow down in) while facing outward instantly relocates the player to the mirrored column on the opposite band, well before reaching the screen edge (see [Tunnel Dash](#tunnel-dash) below)                                                                                                                                                                                                                                                                                                                                    |
...
While `tunnelDash` is owned, `applyTunnelDash` runs once per frame right after `movement`. The trigger zone is `tunnelDashOutwardEdge` — the **same left/right tunnel band** `isGhostTunnelSlow` already uses (`col ≤ leftBand` or `col ≥ cols − rightBand`, sharing one `horizontalTunnelBandEdge` helper), not just the single boundary column. The moment the player's tile is inside that band on a tunnel row **and** `Facing` points outward through that band's edge (left in the left band, right in the right band), they're relocated to `MAZE_COLS - 1 - col` on the same row — the geometric mirror of their current column — landing them in the equivalent spot of the opposite band. This fires several tiles before the screen edge, well ahead of the normal boundary-crossing wrap in `movement`'s `wrapPosition`. A defensive `isWalkable` check on the mirrored destination no-ops the dash if a non-symmetric maze would land the player in a wall (procedurally generated boards are built from a mirrored tiling, so this should not happen in practice). Facing is unchanged, so immediately after the dash the player is moving _into_ the new band, not back out through it, which is what keeps this from firing every frame (no ping-pong).
