...
| `powerWallPass`     | Wall Pass     | Power pellet: walls-only pass-through for `WALL_PASS_MS` (3000); house, exterior, and perimeter wall tiles stay solid; timer-end snap to nearest `playerSolids` walkable center; mild blueward player tint (`PLAYER_WALL_PASS_TINT`) while active                                                 |
