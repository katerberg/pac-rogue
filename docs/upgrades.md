...
| `tunnelDash`        | Tunnel Dash     | Entering a tunnel mouth while facing outward relocates the player to the mirrored tunnel mouth immediately, instead of walking the rest of the tile to the normal wrap point (see [Tunnel Dash](#tunnel-dash) below)                                                                                                                                                                                                                                                                                                                                                                    |
...
While `tunnelDash` is owned, `applyTunnelDash` runs once per frame right after `movement`. If the player's current tile is a tunnel mouth (`tunnelMouthOutwardEdge`) and `Facing` points outward through that mouth (left at the west mouth, right at the east mouth, and the equivalent for a vertical tunnel), the player is relocated straight to the mirrored tunnel-mouth cell center (`oppositeTunnelCell`) that same frame — beating the normal boundary-crossing wrap in `movement`'s `wrapPosition`, which otherwise requires walking to the edge of that tile first. Facing is unchanged, so immediately after the dash the player is moving _into_ the new mouth's row/column, not back out through it, which is what keeps this from firing every frame (no ping-pong).
