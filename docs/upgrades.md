...
While `tunnelDash` is owned, `applyTunnelDash` runs once per frame right after `movement`. The trigger zone is `tunnelDashOutwardEdge` — the **same left/right tunnel band** `isGhostTunnelSlow` already uses (`col ≤ leftBand` or `col ≥ cols − rightBand`, sharing one `horizontalTunnelBandEdge` helper), not just the single boundary column. The moment the player's tile is inside that band on a tunnel row **and** `Facing` points outward through that band's edge (left in the left band, right in the right band), the dash fires: it sweeps and removes every `Pellet` between the player's current column and the near boundary column (inclusive, same row) — folding the count into `pelletProgress` / lifetime collected / SFX exactly like a normal chomp, queuing any swept `PowerPellet` positions for Second Chomp, and resolving `onPowerPellet` effects for any swept power pellets via the same `resolvePowerPelletTrigger` Fruit Power uses — then relocates the player to the **literal opposite tunnel mouth** (col `0` or `MAZE_COLS - 1`, matching where a normal wrap would land them, not a proportional mirror position). This fires several tiles before the screen edge, well ahead of the normal boundary-crossing wrap in `movement`'s `wrapPosition`, while never skipping over a pellet the player would otherwise have walked through (landing exactly at the classic tunnel mouth, rather than mid-band, means nothing is skipped on the arrival side either). The near boundary column is always walkable by construction (`tunnelDashOutwardEdge` requires `hasHorizontalTunnel`, which checks both boundary cells), so no defensive walkability check is needed on the destination. Facing is unchanged, so immediately after the dash the player is moving _into_ the new band, not back out through it, which is what keeps this from firing every frame (no ping-pong).
