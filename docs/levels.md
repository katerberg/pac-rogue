...
| 2     | Procedural 28×34               | Blinky + a randomly chosen Pinky or Inky (see below) | present, no effect | Pick-one upgrade-choice modal (if any upgrade is still eligible), then advance to level 3           |
| 3-7   | Procedural 28×34               | All four (Blinky, Pinky, Inky, Clyde)                | present, no effect | Pick-one upgrade-choice modal (if eligible), then advance to the next level                         |
| 8     | Procedural 28×34               | All four                                             | present, no effect | Pick-one upgrade-choice modal (if eligible), then a `RUN COMPLETE` screen and return to `MenuScene` |
...
Fruit still spawns/despawns exactly as before (layout-scaled pellet thresholds, 10s lifetime, absent on level 1). Picking it up plays the munch SFX and removes it — nothing else. The upgrade-choice reward instead comes from **clearing a level** (2 through 8); see [docs/upgrades.md](./upgrades.md).
