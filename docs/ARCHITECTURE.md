...
5. Countdown starts at 9999 and decrements once per 100ms of real delta after the first direction input; clamps and stays at 0. Score for a successful clear is remaining time at the clear frame.
...
- Top-right `Time` countdown (9999, −1/100ms after first input, clamp at 0). Clearing all pellets appends remaining time as score to unbounded `localStorage` run history (`pac-rogue.run-history.v1`); no on-canvas history UI.
