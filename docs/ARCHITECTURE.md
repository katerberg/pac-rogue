...
2. `create()`: optional `?maze=maze1|maze2` (else 50/50 random) and `?level=` (else 1), then board spawn (world, walls, pellets, player, ghosts from `ghostKindsForLevel(levelIndex)` — Blinky → Pinky → Inky → Clyde, one new kind per level through 4, full set thereafter), HUD (lifetime Collected, Time, lives icons, upgrade strip), `LEVEL N` fade banner, `lives = START_LIVES`, `RunUpgrades` from URL flags (`forceUpgrade` / `enableUpgrade` — see [README Flags](../README.md#flags)), siren.
