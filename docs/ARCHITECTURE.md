...
1. `preload()`: pac-man frames, pellet + power-pellet art, Blinky + Pinky, bonus fruit art, SFX.
...
8. Pellet clear still records score + level-complete SFX; power pellets play both munches. `render` draws pipes, pac-man chomp, `dot.png` / `power-pellet.png`, Blinky, Pinky, bonus fruit, and tunnel twin.
