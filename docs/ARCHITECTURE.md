...
PlayScene.update →
  ...
  playerInput →
  tickGhostRelease + ghostRelease (boardCollected + afterLifeRelease gates Inky/Clyde) →
...
- Start with 3 lives (pac icons bottom-left); lives carry across level advances. Blinky, Pinky, Inky, and Clyde: shared house spawn; Blinky/Pinky time release after first input (0.1s / 5s — tunable); Inky leaves at board-scaled pellets (maze1 baseline 30) on the first life of a board, or after a 7s post-life time gate after a life loss; Clyde leaves at board-scaled pellets on the first life of a board, or after a 9s post-life time gate after a life loss; chase-first arcade scatter/chase waves; Blinky Cruise Elroy; Inky Blinky-vector chase + SE scatter; tunnel slowdown; ghosts gain +10% resolved speed per level index. Circle overlap spends a life (hold → reset actors / ready pause → resume) or last-life Game Over (hold → append high-score run → fade → `GAME OVER` + lifetime collected → menu) unless freeze walk-through is active.
