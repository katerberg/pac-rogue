...
src/
  ...
  domain/                     # Pure helpers (no Phaser, no bitecs world APIs)
    ...
    mazeTiling.ts             # 9×10 mirrored polyomino tiling solver (procedural)
    mazeGenerate.ts           # tiling → 28×31 ASCII + board seed / level≥2 selection
    playfieldBounds.ts        # PLAYFIELD_WIDTH/HEIGHT (no maze import)
    runLevel.ts               # ?level= URL parse
    levelRules.ts             # per-level ghost speed mul, unlock roster, mode wave schedule
...
PlayScene --pellet clear--> level-complete SFX → brief freeze → next procedural maze (carry lives/upgrades/lifetime)
...
2. `create()`: optional `?maze=maze1|maze2|mazeSmall` (else level 1 → `mazeSmall`; level 2+ → procedural ASCII via `mazeGenerate`) and `?level=` (else 1), then board spawn (world, walls, pellets, player, ghosts from `ghostKindsForLevel(levelIndex)` — Blinky → Pinky → Inky → Clyde, one new kind per level through 4, full set thereafter), HUD (lifetime Collected, Time, lives icons, upgrade strip), `LEVEL N` fade banner, `lives = START_LIVES`, `RunUpgrades` from URL flags (`forceUpgrade` / `enableUpgrade` — see [README Flags](../README.md#flags)), siren.
3. Rectangular ASCII layouts (classic `maze1` / `maze2` are 28×31; level-1 `mazeSmall` is 22×21; generated boards are 28×31). Size band and fit gates: [docs/maze-constraints.md](./maze-constraints.md). Ghost house / door are carved in ASCII (`=` door, `H` floor; empty corridor `-` or space; optional `P` spawn). House spawn/exit and fruit cell are **derived** from ASCII; tunnels from edge walkability. `getActiveLayout().playerSolids` blocks the house; `ghostSolids` allows it. Helpers read the active layout set by `activateLayout` / `activateAsciiLayout` (syncs `MAZE_COLS` / `TILE_SIZE` / offsets).
...
- Regular pellets (`dot.png`) and power pellets (`power-pellet.png` on `@` cells) on playable cells; touching removes them, plays pickup SFX (both munches for power pellets; volumes from SFX settings), and increments a top-left **lifetime** `Collected` counter (board-local count drives Inky/Clyde/fruit/clear). Looping siren plays during `PlayScene` until clear, catch, or shutdown (volume from music settings); clearing all pellets plays level-complete SFX then advances to a procedural board; catch plays death SFX then life-loss reset (or Game Over on the last life).
