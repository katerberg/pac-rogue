...
PlayScene.update →
  ...
  tickFreeze + tickScatterBurst + tickInvuln → applyPlayerSpeed → applyGhostSpeed (level mul × upgrade mul + freeze; skips inHouse) →
  movement →
  ...
  render (ghost freeze tint; player invuln gold blink) →
...
- Power pellets are inert unless an owned upgrade reacts (`powerPelletFreeze`, `scatterBurst`, `ghostRecall`, `warpTop` — see [docs/upgrades.md](./upgrades.md)). No arcade fright / eatable ghosts yet.
