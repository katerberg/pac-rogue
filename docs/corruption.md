...
- [`src/domain/corruption.ts`](../src/domain/corruption.ts): `CorruptionId` union, `CORRUPTION_DEFS`
  ...
  `RunCorruption` state (`ghostKind`, `type`, per-corruption cycle/flash timers, `trail` /
  `dropperTrail` tile FIFOs).
...
| `pelletDropper`       | Tracks its own trailing 3 tiles (`PELLET_DROPPER_TRAIL_LEN`, shares the `pushTrailTile` FIFO helper with Slime Trail) and, every 10000ms (`PELLET_DROPPER_INTERVAL_MS`) while `pelletsRemaining > 0`, flashes for 400ms then spawns a real `Pellet` entity on each still-walkable, still-unoccupied trailing tile — these count toward the level's win condition (`addPelletsToProgress` bumps `pelletsRemaining` only, not `boardCollected`), so the player must clear them too. |
